...
- Секреты загружаются из переменных окружения (`.env` — в `.gitignore`)
- Сессия и ключ активации зашифрованы через Windows DPAPI (CryptProtectData)
- Ключ активации привязан к железу (CPU+motherboard+disk)
- В логах и метаданных ключ заменён на SHA256-хеш (первые 8 символов)
- Пользовательские данные (`settings.json`, `macros.json`, `profiles/`) — в `.gitignore`
