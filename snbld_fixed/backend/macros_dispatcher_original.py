...
class MacroDispatcher:
    ...
    def on_macro_finished(self, macro_name: str):
        ...
        Сбрасывает cast_lock с задержкой из cast_lock_margin.
        """
        with self.lock:
            # Используем cast_lock_margin как задержку после конца макроса
            finish_delay = self.backend.settings.get("cast_lock_margin", 0.4)
            
            if finish_delay > 0:
                # Запускаем таймер для сброса cast_lock
                def delayed_unlock():
                    import time
                    time.sleep(finish_delay)
                    with self.lock:
                        self.cast_lock_until = 0.0
                        logger.debug(f"🔓 {macro_name}: cast_lock сброшен (после задержки {finish_delay}с)")
                
                import threading
                unlock_thread = threading.Thread(target=delayed_unlock, daemon=True)
                unlock_thread.start()
                logger.debug(f"⏳ {macro_name}: cast_lock сбросится через {finish_delay}с (из cast_lock_margin)")
            else:
                # Без задержки — сбрасываем сразу
                self.cast_lock_until = 0.0
