...
class SkillMacro(SimpleMacro):
    ...
    def _run(self):
        ...
        # ✅ last_used будет обновлён ТОЛЬКО в конце _run() после ЗАВЕРШЕНИЯ макроса
        # Диспетчер уже проверил cooldown в request_macro() - не трогаем last_used здесь!
        ...
        if approach_used or user_was_moving:
            ...
            # ❌ УДАЛЕНО: Дублирующая установка cast_lock_until!
            # Блокировка каста уже установлена в request_macro() атомарно и безопасно.
            # Повторная установка здесь создаёт гонку и смещает время блокировки.
            # Диспетчер устанавливает блокировку ПЕРЕД запуском макроса - этого достаточно!
            
            _get_logger().debug(f"[SKILL] 🔒 Блокировка каста уже установлена диспетчером (не дублируем)")
            ...
            if use_castbar_detection and self.app.castbar_point:
                ...
                while time.time() - start_wait < timeout and self.running and not self.stop_event.is_set():
                    ...
                    if self.app.is_castbar_visible():
                        ...
                        # ✅ НЕ записываем last_used здесь - только в конце _run()!
                        # cast_detected = True достаточно для выхода из цикла
                ...
                if not cast_detected:
                    _get_logger().warning(f"[SKILL] ⚠️ Полоска не обнаружена за {timeout} сек, НО выполняем Шаг 3 (ресвап) для безопасности!")
                    # ✅ ВАЖНО: Не прерываем макрос! Выполняем шаг 3 даже без кастбара
                    # Это предотвращает ситуацию когда игрок остаётся в уязвимом сете
            else:
            ...
            # last_used обновляется ТОЛЬКО в конце _run() - не здесь!
        ...
        # Это ЕДИНСТВЕННОЕ место где записывается last_used - никаких дублирований!
        with self.cooldown_lock:
            self.last_used = time.time()
            _get_logger().debug(f"[SKILL] 💾 {self.name}: last_used = {self.last_used:.2f} (КД {self.cooldown:.2f}с)")
        ...
        # ✅ Сбрасываем cast_lock НЕМЕДЛЕННО после завершения макроса
        # cast_lock_margin используется диспетчером для установки блокировки ПЕРЕД кастом,
        # а не для задержки сброса ПОСЛЕ завершения!
        if hasattr(self, 'app') and hasattr(self.app, 'dispatcher'):
            self.app.dispatcher.on_macro_finished_immediate(self.name)