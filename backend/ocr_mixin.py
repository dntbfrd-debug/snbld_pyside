...
class OCRMixin:
    ...
    def request_sync_distance(self, target_type="mob"):
        if self.target_reader and self.target_reader.worker:
            try:
                distance = self.target_reader.worker.sync_read(target_type)
                if distance is not None and 0.5 <= distance <= 200:
                    self._target_distance = distance
                    self._last_ocr_numbers = []
                    logger.debug(f"[OCR] sync: {target_type} = {distance:.1f}м")
                    return distance
            except Exception as e:
                logger.error(f"[OCR] sync read error: {e}", exc_info=True)
            return None
        ...
        import pytesseract
        import re
        from tesseract_reader import ensure_tesseract
        try:
            area = self._settings.get(f"{target_type}_area")
            if not area:
                return None
            ...
            for num in numbers:
                try:
                    ...
                    if 0.5 <= distance <= 200:
                        self._target_distance = distance
                        self._last_ocr_numbers = []
                        logger.debug(f"[OCR] sync standalone: {target_type} = {distance:.1f}м")
                        return distance
                except ValueError:
                    continue
            return None
        except Exception as e:
            logger.error(f"[OCR] sync standalone error: {e}", exc_info=True)
            return None
